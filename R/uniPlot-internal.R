...
function(libname, pkgname) {
    Uver <- read.dcf(file=system.file("DESCRIPTION", package=pkgname),
                      fields="Version")
    packageStartupMessage(paste(pkgname, Uver))
    packageStartupMessage("
\n\tStart uniPlot with 'uniPlot()'.

\n\tNOTE: uniPlot creates two new environments which must be placed before 'package:graphics' (type 'search()' to verify that).")
}
